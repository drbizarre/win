...
<script src="<?php echo base_url(); ?>assets/js/jquery.validate.min.js"></script> 
    ...
    <script type="text/javascript" charset="utf-8" language="javascript" src="<?php echo base_url(); ?>assets/js/moment.min.js"></script>
    <script type="text/javascript" charset="utf-8" language="javascript" src="<?php echo base_url(); ?>assets/js/daterangepicker.js"></script>

<!-- Script for this page -->
<script type="text/javascript">
function add2cart(cotizacion,tipo,qty,concepto,descripcion,precio,id){
    $.get( 'http://crm.creactivo.mx/index.php/cotizacion/additem', { cotizacion: cotizacion, tipo: tipo, qty: qty, concepto: concepto, descripcion:descripcion, costo:precio,id_tipo:id },function(data){
    var domain = '<?php echo site_url("cotizacion/carrito"); ?>';
    var url = domain+"/"+cotizacion; 
...
$(function () {
$('#fechas').daterangepicker(null, function(start, end, label) {
                    console.log(start.toISOString(), end.toISOString(), label);
                  });  
  $("#empresa").select2({allowClear: true}); 
  $("#otrob").click(function(event) {
     var cotizacion = $("#id").val();
     var qty = $("#qtyotro").val();
     $("#qtyotro").attr("disabled",true);
     var concepto = $("#otroc").val();
     $("#otroc").attr("disabled",true);
     var precio = $("#preciotro").val();
     $("#preciotro").attr("disabled",true);
    add2cart(cotizacion,'producto',1,concepto,'',precio,94);
    $("#preciotro").attr("disabled",false);
    $("#otroc").attr("disabled",false);
    $("#qtyotro").attr("disabled",false);
...
$('#cliente').change(function() {
    var id = $(this).val();
    var domain = '<?php echo site_url("clientes/porid"); ?>';
    var url = domain+"/"+id; 
     $.get(url,{},function(data){
      $("#correoa").show();
      $("#correo").val(data);
      $("#productos").attr('disabled',false);
      $("#paquetes").attr('disabled',false);
     });    
