...
class User extends CI_Model {
	...
	function getAllUsersByXREF() {
		$query = $this->db->query("SELECT user.user_id, user.first_name, user.last_name, user.user_level, roles.nombre as role FROM user INNER JOIN roles ON user.user_level = roles.id ORDER BY user.first_name ASC");
		return $query->result(); 
	}
	function getFiles($id) {		
		...
		$this->db->where('archivos.id_empleado', $id);
		$this->db->order_by('archivos.id', 'ASC');
		$query = $this->db->get();
		return $query->result();
	}	
		
	function getAllByEmpresa() {		
		$this->db->select('user.user_id,empresas.nombre as empresa, user.first_name,user.last_name');
		$this->db->from('user');
		$this->db->join('empresas', 'user.empresa = empresas.id');		
	...
	function updateUser($id,$username,$codigo,$saldo,$unidad,$project,$email,$first_name,$last_name,$role) {
			$data = array(
               'username' => $username,
               'codigo' => $codigo,
               'saldo' => $saldo,
	...
	function updateUserPass($id,$username,$codigo,$saldo,$unidad,$project,$email,$password,$first_name,$last_name,$role) {
			$data = array(
               'username' => $username,
               'codigo' => $codigo,
               'project' => $project,
	...
	function verify_and_get_user($username, $password) {
		$this->db->select('user_id, unidad,empresa,username, first_name, last_name, user_level');
