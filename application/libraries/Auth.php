...
class Auth {
	...
	function create_user($page) {
		...
		$this->CI->form_validation->set_rules('last_name', 'Last Name', 'required|trim|max_length[25]|xss_clean');
		...
		if ($this->CI->form_validation->run() == FALSE) {
			...
			$this->CI->load->model('regiones_model');
			$data['roles'] = $this->CI->roles_model->getAll();
			$data['empresas'] = $this->CI->empresas_model->getAll();
			$data['regiones'] = $this->CI->regiones_model->getAll();
			

			$this->CI->load->view('template', $data);	

		} else {
			$r = $this->CI->input->post("region");
			$regiones = implode(",",$r);
			...
			$new_user_array = array(
								...
								'empresa' => $this->CI->input->post('empresa'),
								'unidad' => $regiones
	...
	function update_user($page) {
		...
		if ($this->CI->form_validation->run() == FALSE) {
			...
			$codigo = $this->CI->input->post('codigo');

			$unidad = $this->CI->input->post('unidad');
			...
			if ($codigo != NULL || FALSE) :

				$updated_user = array('codigo' => $codigo);
			...
			if ($unidad != NULL || FALSE) :

				$updated_user = array('unidad' => $unidad);
			...
			if ($project != NULL || FALSE) :
				...
				$updated_user = array('project' => $project);
